...
class USDZViewController: UIViewController {
    ...
    private func runSession() {
        ...
        ARlog.start(sceneView, sessionName: "USDZView")
    }
...
extension USDZViewController: ARSCNViewDelegate {
    ...
    func renderer(_ renderer: SCNSceneRenderer, didAdd node: SCNNode, for anchor: ARAnchor) {
        ...
        ARlog.debug("\(self.classForCoder)/\(#function) anchor:\(anchor), node: \(node), node geometry: \(String(describing: node.geometry))")
    ...
    func renderer(_ renderer: SCNSceneRenderer, didRemove node: SCNNode, for anchor: ARAnchor) {
        ...
        ARlog.debug("\(self.classForCoder)/" + #function)
        guard let planeAnchor = anchor as? ARPlaneAnchor else { return }
